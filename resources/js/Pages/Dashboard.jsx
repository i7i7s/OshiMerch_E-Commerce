...
import { Plus, Package, Edit3, Trash2, ShoppingBag } from 'lucide-react';
        ...
        >
            ...
            <div className="flex-1 min-w-0">
                <Link
                    ...
                    className="font-semibold text-surface-800 text-sm leading-snug line-clamp-1 hover:text-primary-600 transition-colors"
                >
                    {listing.title}
                </Link>
                <div className="flex items-center gap-2 mt-1 flex-wrap">
                    {teamInfo && (
                        <span className={`px-1.5 py-0.5 rounded text-[10px] font-bold text-white ${teamInfo.bg}`}>
                            {teamInfo.label}
                        </span>
                    )}
                    <span className={`px-2 py-0.5 rounded-lg text-[10px] font-semibold ${STATUS_STYLE[listing.status] || STATUS_STYLE.Available}`}>
                        {listing.status}
                    </span>
            ...
            <div className="flex items-center gap-2 shrink-0">
                <Link
                    ...
                    className="w-9 h-9 rounded-xl flex items-center justify-center border border-surface-200 text-surface-600 hover:bg-surface-100 hover:text-primary-600 transition-all"
                ...
                <button
                    ...
                    className="w-9 h-9 rounded-xl flex items-center justify-center border border-surface-200 text-surface-600 hover:bg-red-50 hover:border-red-200 hover:text-red-500 transition-all"
        ...
        <AuthenticatedLayout showFooter>
            ...
            <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
                        ...
                        <div className="flex-1 min-w-0">
                            ...
                            {user.oshi_member_name && (
                                <p className="text-white/80 text-sm mt-1">
                                    Oshi: <span className="font-semibold text-white">{user.oshi_member_name}</span>
                                </p>
                            )}
                        </div>
                        <Link
                            href={route('listings.create')}
                            className="shrink-0 inline-flex items-center gap-2 px-5 py-3 rounded-xl bg-white text-primary-600 font-bold text-sm hover:bg-surface-50 transition-all shadow-elevated hover:shadow-xl hover:scale-[1.02] active:scale-[0.98]"
                        >
                ...
                <div className="grid grid-cols-2 sm:grid-cols-4 gap-4 mb-8">
                    {[
                        { label: 'Total Listing', value: listings.length, icon: Package, color: 'text-primary-500', bg: 'bg-primary-50' },
                        { label: 'Listing Aktif', value: activeCount, icon: ShoppingBag, color: 'text-green-500', bg: 'bg-green-50' },
                        { label: 'Transaksi', value: 0, icon: ShoppingBag, color: 'text-secondary-500', bg: 'bg-secondary-50', note: 'Fase 3' },
                        { label: 'Pesan Masuk', value: 0, icon: ShoppingBag, color: 'text-amber-500', bg: 'bg-amber-50', note: 'Fase 3' },
                    ].map((stat, i) => (
                        <motion.div
                            key={i}
                            initial={{ opacity: 0, y: 12 }}
                        ...
                        >
                            ...
                            <p className="text-2xl font-bold font-display text-surface-900">
                                {stat.value}
                                {stat.note && <span className="text-xs text-surface-400 font-normal ml-1">{stat.note}</span>}
                            </p>
                ...
                <div>
                    ...
                    {listings.length === 0 ? (
                        <motion.div
                            initial={{ opacity: 0, y: 12 }}
                            animate={{ opacity: 1, y: 0 }}
                            className="py-16 rounded-2xl bg-white border border-dashed border-surface-300 text-center"
                        >
                            ...
                            <h3 className="text-base font-bold text-surface-700 mb-1.5">Belum ada listing</h3>
                            <p className="text-sm text-surface-500 mb-5">Mulai jual merchandise JKT48 kamu sekarang!</p>
                            <Link
                                href={route('listings.create')}
                        ...
                        <div className="space-y-3">
                            {listings.map((listing) => (
                                <MyListingRow key={listing.id} listing={listing} onDelete={handleDelete} />
                            ))}
                        </div>
                    )}
                </div>
